...
def get_wordnet_synonyms(word, wordnet, language="eng"):
    """Return stricter one-word WordNet synonyms.

    Optimization:
    1. Remove very rare WordNet lemmas.
    2. Remove candidates whose length is too different from the original word.
    3. Keep only alphabetic one-word candidates.
    """
    original = word.lower()
    synonyms = set()
    ...
    for synset in wordnet.synsets(word, lang=language):
        for lemma in synset.lemmas(lang=language):
            ...
            # WordNet lemma count is a simple frequency signal.
            # count == 0 often means the candidate is rare or unnatural.
            if lemma.count() == 0:
                continue

            # Avoid replacements like exciting -> sex.
...
def is_modifiable(token):
    ...
    if token_lower in STOPWORDS:
        return False
    if len(token_lower) < min_word_len:
        return False
...
def rank_words(tokens, true_label, state):
    ...
    modifiable_indices = [
            ...
            if is_modifiable(token, state["min_word_len"])
    ]
...
def attack_one(text, true_label, base_state):
    ...
    modifiable_count = sum(
        is_modifiable(token, state["min_word_len"])
        for token in tokens
    )

    max_changes = max(
        1,
        int(modifiable_count * state["max_change_ratio"] + 0.999),
    )

    for index in word_order:
        if len(result["changed_words"]) >= max_changes:
            break
...
def parse_args():
    ...
    parser.add_argument("--min-word-len", type=int, default=1)
    parser.add_argument("--max-change-ratio", type=float, default=1.0)
